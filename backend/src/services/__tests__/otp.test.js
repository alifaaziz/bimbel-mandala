...
describe('OtpService', () => {
  ...
  describe('sendUserVerificationOtp', () => {
    it('throws if rate limited', async () => {
      mockPrisma.otp.count.mockResolvedValueOnce(3);
      await expect(OtpService.sendUserVerificationOtp('A', 'a@mail.com', 'user1'))
    ...
    it('sends OTP if not rate limited', async () => {
      ...
      expect(mockSendOtpEmail).toHaveBeenCalledWith('a@mail.com', '123456', expect.any(Date));
    });
  });
  ...
  describe('verifyOtp', () => {
    it('throws if otp not found', async () => {
      ...
      await expect(OtpService.verifyOtp({ email: 'a@mail.com', otp: '123456' }))
        .rejects.toThrow(HttpError);
    });
    ...
    it('verifies OTP and returns user with token', async () => {
      ...
      mockPrisma.user.findUnique.mockResolvedValueOnce({
        id: 'user1', email: 'a@mail.com', name: 'A', googleId: null
      });
      mockPrisma.$transaction.mockImplementationOnce(async (fn) => {
        await fn({
      ...
      const result = await OtpService.verifyOtp({ email: 'a@mail.com', otp: '123456' });
