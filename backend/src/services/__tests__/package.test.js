...
describe('BimbelPackageService', () => {
    ...
    describe('getAllBimbelPackages', () => {
        ...
        it('uses default page and pageSize if not provided', async () => {
            mockPrisma.bimbelPackage.findMany.mockResolvedValueOnce([
                {
                    name: 'Paket Default',
                    level: 'SMA',
        ...
        it('returns empty data if no packages', async () => {
            mockPrisma.bimbelPackage.findMany.mockResolvedValueOnce([]);
    ...
    describe('createBimbelPackage', () => {
        ...
        it('returns correct structure and types', async () => {
            const fakeReturn = {
        ...
        it('should return groupType with correct maxStudent as string', async () => {
            const fakeReturn = {
                id: '0196dc4c-52bb-7f60-9853-b0b12c6da401',
                name: 'Matematika Mudah',
                level: 'SMA',
    ...
    describe('updateBimbelPackageStatus', () => {
        it('updates isActive if all schedules have attendance', async () => {
        ...
        it('does nothing if not all schedules have attendance', async () => {
            mockPrisma.class.findMany.mockResolvedValueOnce([
                {
                    schedules: [
                        { attendances: [] },
    ...
    describe('getBimbelPackagesByPopularity', () => {
        it('returns sorted packages', async () => {
    ...
    describe('getRunningPrograms', () => {
        it('returns running classes', async () => {
            mockPrisma.class.findMany.mockResolvedValueOnce([
                {
                    id: 1,
        ...
        it('returns null bimbelPackageName if order or bimbelPackage is null', async () => {
            mockPrisma.class.findMany.mockResolvedValueOnce([
                {
                    id: 10,
                    tutor: { name: 'Budi', tutors: [{ gender: 'Male' }] },
        ...
        it('returns empty array if no running classes', async () => {
            mockPrisma.class.findMany.mockResolvedValueOnce([]);
            const result = await BimbelPackageService.getRunningPrograms();
    ...
    describe('getMyPackages', () => {
        it('returns packages for tutor', async () => {
            mockPrisma.bimbelPackage.findMany.mockResolvedValueOnce([
                {
                    id: 'pkg1',
                    name: 'Paket Tutor',
        ...
        it('returns groupType with discPrice null if discPrice is null', async () => {
            mockPrisma.bimbelPackage.findMany.mockResolvedValueOnce([
                {
                    id: 'pkgX',
    ...
    describe('getMyPackageById', () => {
        it('returns package for tutor', async () => {
        ...
        it('returns null if tutor has no such package', async () => {
            mockPrisma.bimbelPackage.findFirst.mockResolvedValueOnce(null);
            const result = await BimbelPackageService.getMyPackageById('notfound', { id: 1, role: 'tutor' });
            expect(result).toBeNull();
        });
    ...
    describe('getMyProgramsStatistics', () => {
        it('returns stats for siswa', async () => {
        ...
        it('returns stats for tutor', async () => {
            mockPrisma.class.findMany.mockResolvedValueOnce([
                { status: 'berjalan' },
                { status: 'selesai' },
                { status: 'selesai' }
        ...
        it('throws if role not supported', async () => {
        ...
        it('returns 0 for empty studentClass', async () => {
            mockPrisma.studentClass.findMany.mockResolvedValueOnce([]);
            const result = await BimbelPackageService.getMyProgramsStatistics({ id: 1, role: 'siswa' });
            expect(result).toEqual({ runningClasses: 0, completedClasses: 0 });
        });

        it('returns 0 for empty tutor classes', async () => {
            mockPrisma.class.findMany.mockResolvedValueOnce([]);
            mockPrisma.bimbelPackage.count.mockResolvedValueOnce(0);
            const result = await BimbelPackageService.getMyProgramsStatistics({ id: 1, role: 'tutor' });
