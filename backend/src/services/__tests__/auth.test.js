  ...
  describe('login', () => {
    it('should throw if notVerifiedUser exists', async () => {
      mockPrisma.user.findUnique
        .mockResolvedValueOnce(null) // user (verified)
        .mockResolvedValueOnce({ id: 1 }); // notVerifiedUser
    ...
    it('should throw if user not found', async () => {
      mockPrisma.user.findUnique
        .mockResolvedValueOnce(null) // user (verified)
        .mockResolvedValueOnce(null); // notVerifiedUser
      await expect(AuthService.login({ email: 'a@mail.com', password: 'pw' }))
    ...
    it('should throw if password not match', async () => {
      mockPrisma.user.findUnique
        .mockResolvedValueOnce({ id: 1, password: 'hashed', name: 'A', email: 'a@mail.com', googleId: null }) // user
        .mockResolvedValueOnce(null); // notVerifiedUser
